...
и дописать его в **уже существующий** `title` имени трека:

```ts
row.createSpan({ cls: "sw-pl-name", text: f.basename, title: `${f.path}\n${t("rowDragHint")}` });
```

**На саму строку подсказку вешать нельзя**, хотя это и напрашивается. Obsidian
рисует тултип из `aria-label` по наведению на любого потомка, так что ярлык на
строке столкнулся бы сразу с двумя вещами внутри неё: нативным тултипом пути у
имени и собственным `aria-label` у флажка маркера. Вдобавок одинаковый
`aria-label` на всех строках отнимает у них имя: `role="button"` без ярлыка
читается по своему тексту (номер и название), а с ярлыком все строки
